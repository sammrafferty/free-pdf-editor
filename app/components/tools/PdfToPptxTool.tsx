  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= doc.numPages; i++) {
        ...
        // Determine slide dimensions (in inches)
        const aspectRatio = viewport.width / viewport.height;
        const slideW = 10;
        const slideH = slideW / aspectRatio;

        const slide = pptx.addSlide();
        pptx.defineLayout({ name: `Page${i}`, width: slideW, height: slideH });
      ...
      // Set layout to match first page
    ...
    <div>
      {!file ? (
        ...
        <div className="space-y-5">
              ...
              {([
                ...
                <button
                  ...
                  className={`flex-1 py-3 rounded-xl border text-sm font-medium transition-colors ${
                    quality === opt.key ? "text-white" : "bg-white border-gray-200 text-gray-600 hover:border-red-300"
                  }`}
                  style={quality === opt.key ? { backgroundColor: accentColor, borderColor: accentColor } : {}}
                >
                  <div>{opt.label}</div>
                  <div className={`text-xs mt-0.5 ${quality === opt.key ? "text-red-100" : "text-gray-400"}`}>{opt.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Text overlay option */}
          <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100">
            ...
            <div>
              <span className="text-sm font-medium text-gray-700">Include text as slide notes</span>
              <p className="text-xs text-gray-400">Extracted text will be added to speaker notes for searchability</p>
            </div>
          </label>
